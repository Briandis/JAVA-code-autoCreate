...
def create_xml_mapper(config: dict):
    ...
    data += _create_xml_delete_by_key_where_block(config) + '\n'
    ...
    data += _create_xml_update_by_key_where_block(config) + '\n'
...
def __check_res_filed_repeat(config: dict):
    ...
    if config.get("oneToOne"):
        for obj in config.get("oneToOne"):
            ...
            one_to_one_dict[obj["tableName"]] = {
                "attr": __table_filed_list(obj),
                "className": obj["className"],
                "path_pojo": obj["path_pojo"],
                "foreign_key": obj["foreign_key"]
            }
            if obj["tableName"] == config["tableName"]:
                table_my = True
    if config.get("oneToMany"):
        for obj in config.get("oneToMany"):
            ...
            one_to_many_dict[obj["tableName"]] = {
                "attr": __table_filed_list(obj),
                "className": obj["className"],
                "path_pojo": obj["path_pojo"],
                "foreign_key": obj["foreign_key"]
    ...
    if config.get("manyToMany"):
        for obj in config.get("manyToMany"):
            if obj["many"]["tableName"]:
                ...
                if obj["many"]["tableName"] == config["tableName"]:
                    table_my = True
    # 替换
    ...
    for table in tables:
        # 原tables中attr列表中装着全部主键和其他字段，现在需要从中提取主键并删除，将其装配至key中
        # 提取主键并从中移除
...
def __create_result_map(config: dict) -> str:
    ...
    obj_table = set()
    obj_table.add(config["className"])

    table_my = config.get("table_my")
    table_name = config["tableName"]

    config = config.get("bean_info")

    if config.get("oneToOne"):
        for obj in config.get("oneToOne"):
            if table_my and obj["tableName"] == table_name:
                obj["tableName"] += "1"
                obj["key"]["filed"] += "1"
                for attr in obj["attr"]:
                    attr["filed"] += "1"
            data += f'{tag}<resultMap type="{config["path_pojo"]}.{config["className"]}" id="{config["resultMapName"]}OneToOne{obj["className"]}">\n'
            data += __create_method_result_map_key_value(config, 2)
            data += __create_result_map_one_to_one(obj, 2)
            data += f'{tag}</resultMap>\n\n'
            if obj["className"] not in obj_table:
                data += f'{tag}<resultMap type="{obj["path_pojo"]}.{obj["className"]}" id="res{obj["className"]}">\n'
                data += __create_method_result_map_key_value(obj, 2)
                data += f'{tag}</resultMap>\n\n'
            obj_table.add(obj["className"])
    ...
    if config.get("oneToMany"):
        for obj in config.get("oneToMany"):
            if table_my and obj["tableName"] == table_name:
                ...
                for attr in obj["attr"]:
                    attr["filed"] += "1"
            data += f'{tag}<resultMap type="{config["path_pojo"]}.{config["className"]}" id="{config["resultMapName"]}OneToMany{obj["className"]}">\n'
            data += __create_method_result_map_key_value(config, 2)
    ...
    if config.get("manyToMany"):
        for obj in config.get("manyToMany"):
            if table_my and obj["many"]["tableName"] == table_name:
                obj["many"]["tableName"] += "1"
                obj["many"]["key"]["filed"] += "1"
                for attr in obj["many"]["attr"]:
                    attr["filed"] += "1"
            data += f'{tag}<resultMap type="{config["path_pojo"]}.{config["className"]}" id="{config["resultMapName"]}ManyToMany{obj["many"]["className"]}">\n'
            data += __create_method_result_map_key_value(config, 2)
            data += __create_result_map_one_to_many(obj["many"], 2)
            data += f'{tag}</resultMap>\n\n'
            if obj["many"]["className"] not in obj_table:
                data += f'{tag}<resultMap type="{obj["many"]["path_pojo"]}.{obj["many"]["className"]}" id="res{obj["many"]["className"]}">\n'
                data += __create_method_result_map_key_value(obj["many"], 2)
                data += f'{tag}</resultMap>\n\n'
            obj_table.add(obj["many"]["className"])
...
def __create_xml_block_one_to_x_select(config: dict, indent: int, param=True, temp_key_word=""):
    tap = "\t" * indent
    if param:
    ...
    for attr in config["attr"]:
        if "Date" == attr.get("type"):
            data += f'{tap}<if test="{name}{attr["attr"]}!=null">AND DATE({table}{attr["filed"]})=DATE(#{{{name}{attr["attr"]}}})</if>\n'
        else:
            data += f'{tap}<if test="{name}{attr["attr"]}!=null">AND {table}{attr["filed"]}=#{{{name}{attr["attr"]}}}</if>\n'
    data += f'{lost}'
    data += __create_xml_block_fuzzy_search_if(config, indent, table, temp_key_word)
    return data
...
def _create_xml_delete_by_key_where_block(config: dict):
    tag = "\t"
    select_block = __create_xml_block_any_select(config, 3, f'{stringUtil.low_str_first(config["className"])}')
    data = f'{tag}<delete id="delete{config["className"]}By{stringUtil.upper_str_first(config["key"]["attr"])}AndWhere">\n'
    data += f'{tag * 2}delete from {config["tableName"]}\n'
    data += f'{tag * 2}<where>\n'
    data += f'{tag * 3}{config["key"]["filed"]}=#{{{config["key"]["attr"]}}}\n'
    data += select_block
    data += f'{tag * 2}</where>\n'
    data += f'{tag}</delete>\n'
    return data
...
def _create_xml_update_by_not_repeat_where_block(config: dict):
    ...
    temp_str = f'condition{config["className"]}.{config["key"]["attr"]}!=null'
    for attr in config["attr"]:
        temp_str += f' or condition{config["className"]}.{attr["attr"]}!=null'
    data += f'{tag * 2}<if test="condition{config["className"]}!=null and ({temp_str})">\n'
...
def _create_xml_update_by_key_where_block(config: dict):
    tag = "\t"
    data = f'{tag}<update id="update{config["className"]}By{stringUtil.upper_str_first(config["key"]["attr"])}AndWhere">\n'
    data += f'{tag * 2}update {config["tableName"]}\n'
    data += f'{tag * 2}<set>\n'
    data += __create_xml_block_update_if(config, 3, f'save{config["className"]}')
    data += f'{tag * 2}</set>\n'
    data += f'{tag * 2}<where>\n'
    data += f'{tag * 3}{config["key"]["filed"]}=#{{save{config["className"]}.{config["key"]["attr"]}}}\n'
    data += __create_xml_block_any_select(config, 3, f'condition{config["className"]}', False) + '\n'
    data += f'{tag * 2}</where>\n'
    data += f'{tag}</update>\n'
    return data
...
def __create_find_or_query_one_to_one(config: dict, select_type: str, inline=True):
    ...
    if "oneToOne" in config:
        ...
        for obj in config["oneToOne"]:
            manny_table = ""
            if obj["className"] in obj_table:
                manny_table = f'On{stringUtil.upper_str_first(obj["key"]["attr"])}'
            obj_table.add(obj["className"])
            if config["replacementMapping"]:
                select_filed = f'\n{tag * 3}<include refid="sql_filed_{config["tableName"]}"/>,\n'
                if config.get("table_my") and obj["tableName"] == config["tableName"]:
                    select_filed += f'{tag * 3}<include refid="sql_filed_{obj["tableName"]}1"/>\n{tag * 2}'
                else:
                    select_filed += f'{tag * 3}<include refid="sql_filed_{obj["tableName"]}"/>\n{tag * 2}'
            else:
                select_filed = "* "
            data = f'{tag}<select id="{select_type}{config["className"]}OneToOne{obj["className"]}{manny_table}" resultMap="res{config["className"]}OneToOne{obj["className"]}">\n'
            if inline:
                temp_sql = ""
                if config.get("table_my"):
                    temp_sql = f' AS {obj["tableName"]}1'
                data += f'{tag * 2}select {select_filed}from {config["tableName"]},{obj["tableName"]}{temp_sql}\n'
                data += f'{tag * 2}<where>\n'
                ...
                if config.get("table_my") and obj["tableName"] == config["tableName"]:
                    temp_sql = f' AS {obj["tableName"]}1'
                    temp_sql2 = "1"
                data += f'{tag * 2}select {select_filed}from {config["tableName"]} LEFT JOIN {obj["tableName"]}{temp_sql} ON {config["tableName"]}.{obj["foreign_key"]} = {obj["tableName"]}{temp_sql2}.{obj["key"]["filed"]}{temp_sql2}\n'
                data += f'{tag * 2}<where>\n'
            data += __create_xml_block_one_to_x_select(config, 3)
            if "keyWord" in obj and "keyWordList" in obj and len(obj["keyWordList"]) > 0:
                data += __create_xml_block_one_to_x_select(obj, 3, temp_key_word="1")
            else:
    ...
    if "oneToOne" in config:
        ...
        for obj in config["oneToOne"]:
            ...
            if obj["className"] in obj_table:
                manny_table = f'On{stringUtil.upper_str_first(obj["key"]["attr"])}'
            obj_table.add(obj["className"])
            data = f'{tag}<select id="count{stringUtil.upper_str_first(select_type)}{config["className"]}OneToOne{obj["className"]}{manny_table}" resultType="int">\n'
            if inline:
...
def __create_link_one_obj(config: dict):
    tag = "\t"
    method_str = ""
    if "oneToOne" in config:
        obj_table = set()
        for obj in config["oneToOne"]:
            ...
            if obj["className"] in obj_table:
                manny_table = f'On{stringUtil.upper_str_first(obj["key"]["attr"])}'
            obj_table.add(obj["className"])
            select_filed = f'{obj["tableName"]}.* '
            data = f'{tag}<select id="linkOne{obj["className"]}{manny_table}" resultMap="res{obj["className"]}">\n'
    ...
    if "oneToMany" in config:
        ...
        for obj in config["oneToMany"]:
            ...
            if config["replacementMapping"]:
                ...
                if config.get("table_my") and obj["tableName"] == config["tableName"]:
                    select_filed_right = f'\n{tag * 4}<include refid="sql_filed_{obj["tableName"]}1"/>\n{tag * 3}'
                else:
            ...
            data = f'{tag}<select id="{select_type}{config["className"]}OneToMany{obj["className"]}{manny_table}" resultMap="res{config["className"]}OneToMany{obj["className"]}">\n'
            ...
            if config.get("table_my") and obj["tableName"] == config["tableName"]:
                temp_sql = f' AS {obj["tableName"]}1'
                temp_sql2 = "1"
            subquery2 = f'{tag * 3}select {select_filed_right}from {obj["tableName"]}{temp_sql}\n'
            subquery2 += f'{tag * 3}<where>\n'
            if "keyWord" in obj and "keyWordList" in obj and len(obj["keyWordList"]) > 0:
                subquery2 += __create_xml_block_one_to_x_select(obj, 4, temp_key_word="1")
            else:
...
def __create_count_find_or_query_one_to_many(config: dict, select_type: str, inline=True):
    ...
    if "oneToMany" in config:
        ...
        for obj in config["oneToMany"]:
            ...
            data = f'{tag}<select id="count{stringUtil.upper_str_first(select_type)}{config["className"]}OneToMany{obj["className"]}{manny_table}" resultType="int">\n'
            subquery1 = f'{tag * 3}select {select_filed_left}from {config["tableName"]}\n'
            subquery1 += f'{tag * 3}<where>\n'
            subquery1 += __create_xml_block_one_to_x_select(config, 4)
            subquery1 += f'{tag * 3}</where>\n'
...
def __create_link_many_obj(config: dict):
    tag = "\t"
    method_str = ""
    if "oneToMany" in config:
        obj_table = set()
        for obj in config["oneToMany"]:
            ...
            data = f'{tag}<select id="linkMany{obj["className"]}{manny_table}" resultMap="res{obj["className"]}">\n'
            subquery1 = f'{tag * 3}select {config["key"]["filed"]} from {config["tableName"]}\n'
            subquery1 += f'{tag * 3}<where>\n'
            subquery1 += __create_xml_block_one_to_x_select(config, 4)
            subquery1 += f'{tag * 3}</where>\n'
            subquery1 += f'{tag * 3}<if test="page!=null">\n'
            ...
            subquery2 = f'{tag * 3}select * from {obj["tableName"]}\n'
            subquery2 += f'{tag * 3}<where>\n'
            if "keyWord" in obj and "keyWordList" in obj and len(obj["keyWordList"]) > 0:
                subquery2 += __create_xml_block_one_to_x_select(obj, 4, temp_key_word="1")
            else:
...
def __create_find_or_query_many_to_many(config: dict, select_type: str, inline=True):
    ...
    if "manyToMany" in config:
        ...
        for obj in config["manyToMany"]:
            ...
            obj_table.add(f'{obj["to"]["className"]}On{obj["many"]["className"]}')
            if config["replacementMapping"]:
                select_filed_main = f'\n{tag * 4}<include refid="sql_filed_{config["tableName"]}"/>\n{tag * 3}'

                if config.get("table_my") and obj["many"]["tableName"] == config["tableName"]:
                    select_filed_join = f'\n{tag * 3}temp_{config["tableName"]}.*,\n{tag * 3}<include refid="sql_filed_{obj["many"]["tableName"]}1"/>\n{tag * 2}'
                else:
            ...
            data = f'{tag}<select id="{select_type}{config["className"]}ManyToManyLink{obj["to"]["className"]}On{obj["many"]["className"]}{manny_table}" resultMap="res{config["className"]}ManyToMany{obj["many"]["className"]}">\n'
            ...
            if "bean_info" in config:
                my_key = config["bean_info"]["key"]["filed"]
            if inline:
                data += f'{tag * 2}select {select_filed_join}from (\n{subquery}{tag * 2}) AS temp_{config["tableName"]},{obj["to"]["tableName"]},{obj["many"]["tableName"]}{temp_sql}\n'
                data += f'{tag * 2}where temp_{config["tableName"]}.{my_key} = {obj["to"]["tableName"]}.{obj["to"]["foreign_key"]}\n'
                data += f'{tag * 2}AND {obj["to"]["tableName"]}.{obj["many"]["foreign_key"]} = {obj["many"]["tableName"]}{temp_sql2}.{obj["many"]["key"]["filed"]}\n'
            else:
                ...
                data += f'{tag * 2}LEFT JOIN {obj["many"]["tableName"]}{temp_sql} On {obj["to"]["tableName"]}.{obj["many"]["foreign_key"]} = {obj["many"]["tableName"]}{temp_sql2}.{obj["many"]["key"]["filed"]}\n'
            if config.get("SQLInjection") == "true":
                data += f'{tag * 2}<if test="SQLInjection!=null">\n'
                data += f'{tag * 3}${{SQLInjection}}\n'
                data += f'{tag * 2}</if>\n'
...
def _create_xml_x_to_x_block(config: dict):
    ...
    method_str += __create_link_one_obj(config)
    # 左外连由左表决定
    ...
    method_str += __create_link_many_obj(config)
    # 左外连最终条数由左表决定
